...
from app.services.attempt_lifecycle import auto_submit_expired
...
async def get_compiler_status():
    """Return judge engine diagnostics (Docker availability + images)."""
    from app.services import docker_executor
    from app.services.execution_service import _engine, LOCAL

    if _engine() == LOCAL:
        return {"engine": "local", "available": True}
    return docker_executor.get_diagnostics()
...
async def _fetch_test_cases(db: AsyncSession, question_id: int, include_hidden: bool) -> list:
    """Fetch the real test cases for a question from the database only."""
...
async def _fetch_question(db: AsyncSession, question_id: int):
    """Fetch a question from the database, or None when it does not exist."""
...
async def _save_code(db: AsyncSession, attempt_id: int, question_id: int, language: str, source_code: str):
    """Persist (or update) the student's draft code for this attempt+question.

    Draft save is best-effort: it is not the graded record, so a persistence
    failure must never fail the run/submit itself.
    """
    try:
        ...
        logger.warning(
            f"Could not save draft code for attempt {attempt_id}, question {question_id}: {e}"
        )
...
async def run_single_case(
    ...
    """Run one sample or custom input without creating a submission."""
...
async def run_code(
    ...
    if not public_test_cases:
        # No sample cases configured: nothing to run against.
        return CodeRunResponse(
...
async def submit_code(
    ...
    if not all_test_cases:
        raise HTTPException(
            status_code=400,
            detail="This question has no test cases configured; submission is not possible",
        )
